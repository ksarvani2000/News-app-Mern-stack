...
function BitcoinArticles({bitcoinData}) {
  ...
  const subscribeHandler = () => {
    alert(`Your will receive notification from ${bitcoinData.source.name}`)
  }
  ...
  return (
    <div className="card">
      ...
      <a href={`mailto:${emailNotifications}?subject=You have subscribed to ${bitcoinData.source.name}`}><button onClick={subscribeHandler}>Subscribe</button></a>
    </div>
    </div>
  );
}
